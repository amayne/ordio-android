 ...
 */
JNIEXPORT jboolean JNICALL Java_com_ordio_player_backend_MP3Wrapper_ninitLib(JNIEnv* env, jobject this)
{	 
	jint err = MPG123_ERR;
 ...
 */
JNIEXPORT void JNICALL Java_com_ordio_player_backend_MP3Wrapper_ncleanupLib(JNIEnv* env, jobject this)
{
	mpg123_delete(mh);
	mpg123_exit();
	mh = NULL;
 ...
 */
JNIEXPORT jstring JNICALL Java_com_ordio_player_backend_MP3Wrapper_ngetError(JNIEnv* env, jobject this)
{
 ...
 * Init one MP3 file
 */
JNIEXPORT jint JNICALL Java_com_ordio_player_backend_MP3Wrapper_ninitMP3(JNIEnv* env, jobject this, jstring filename)
{
	jint err = MPG123_ERR;
 ...
 */
void Java_com_ordio_player_backend_MP3Wrapper_ncleanupMP3(JNIEnv* env, jobject this)
{
	mpg123_close(mh);
}
 ...
 *
 */
JNIEXPORT jboolean JNICALL Java_com_ordio_player_backend_MP3Wrapper_nsetEQ(JNIEnv* env, jobject this, jint ch, jdouble val)
{
 ...
 *
 */
JNIEXPORT void JNICALL Java_com_ordio_player_backend_MP3Wrapper_nresetEQ(JNIEnv* env, jobject this)
{
	mpg123_reset_eq(mh);
}
 ...
 */
JNIEXPORT jobject JNICALL Java_com_ordio_player_backend_MP3Wrapper_ngetAudioInformations(JNIEnv* env, jobject this, jclass clazz)
{
  	// Get our class defined in java project
	clazz = (*env)->FindClass(env, "com/ordio/player/backend/AudioFileInformations");
	if (clazz == 0)
		return NULL;
 ...
 * Decode our MP3 file
 */
JNIEXPORT jint JNICALL Java_com_ordio_player_backend_MP3Wrapper_ndecodeMP3(JNIEnv* env, jobject this, jint inlen, jshortArray jpcm)
{
	...
	/*jshort *tmp = NULL;
	memcpy(tmp, pcm, outlen);
	float multipler = 0.05f;
	size_t i;
	for(i=1; i<outlen; ++i) {
		//y[i] := y[i-1] + α * (x[i] - y[i-1])
		tmp[i] = (jshort) (tmp[i-1] + multipler * (pcm[i] - tmp[i-1]));
	}*/
 ...
 */
JNIEXPORT void JNICALL Java_com_ordio_player_backend_MP3Wrapper_nseekTo(JNIEnv* env, jobject this, jint pos)
{
	if(mh != NULL)
		mpg123_seek(mh, pos, SEEK_SET);
}
JNIEXPORT void JNICALL Java_com_ordio_player_backend_MP3Wrapper_nsetPlaybackRate(JNIEnv* env, jobject this, jint speed)
{
	if(speed > 0) {
		mpg123_param(mh, MPG123_UPSPEED, speed, speed);
	}
	//} else if (speed < 0) {
//		speed = speed * -1;
//		mpg123_param(mh, MPG123_DOWNSPEED, speed, speed);
//	} else {
//		mpg123_param(mh, MPG123_UPSPEED, 1, 1); 
//	}
}
/*
 * Get next sample to be played
 */
JNIEXPORT jlong JNICALL Java_com_ordio_player_backend_MP3Wrapper_ntell(JNIEnv* env, jobject this)
{
	jlong sample;
	if(mh != NULL)
		sample = mpg123_tell(mh);
	return sample;
}
